...
<metadata>
	<logicalModel>
			...
			<table batchInterfaceHistory="{@batchInterfaceHistory}" description="{@description|description}" name="{@name}" batchInterface="{@batchInterface}" label="{@label}" amountOfChanges="{@amountOfChanges}" showInAllTables="{@showInAllTables}" amountOfRecords="{@amountOfRecords}">
				<columns>
					<xsl:for-each select="columns/column">
          			<column generated="{@generated}" description="{@description|description}" name="{@name}" generatedID="{@generatedID}" domain="{@domain}" displayMode="{@displayMode}" label="{@label}" required="{@required}" defaultValue="{@defaultValue|defaultValue}" dbType="{@dbType}">
						<xsl:if test="(@hyperlink = 'true')"> <!-- valuePresenter="{@valuePresenter}" delete a put this 10.2.0 -> 10.x.x -->
							<xsl:attribute name="valuePresenter">
    	...
    	<valuePresenters>
			...
			<valuePresenter name="hyperlink" template="???" handlesClick="true"/><!--<a href='${RDM_WWW}' target='_${RDM_WWWW}'>WWW Link</a>--><!-- delete this 10.2.0 -> 10.x.x -->
		</valuePresenters>		
	...
	<systems>
		<databaseSystems> 
		<xsl:choose>  <!-- old path -->
			<xsl:when test="exists(systems/system/@name)">
	...
	<syncTasks>
		<syncDatabaseTasks>
			<xsl:choose>  <!-- old path -->
				<xsl:when test="exists(syncTasks/syncTask/@name)">
					<xsl:for-each select="syncTasks/syncTask">
					<syncDatabaseTask name="{@name}">
						<syncTaskTables>
		...
		<syncSFTPTasks>
			<syncSFTPTasksE> 
			<!-- this part delete here 10.2.0 -> 10.x.x and use normal second transformation for File export -->		
			<!-- podminka, ze aspon jeden export je zaskrtlej -->   
				<xsl:if test="//logicalModel/tables/table[@batchInterfaceExport !='false']">
				<syncSFTPTaskE filePath="Files/data/FTs/exports" name="batch" fileName="" compression="NONE" targetDirectory="" SFTPserver="">
					<syncSFTPTaskExports>
						<xsl:for-each select="logicalModel/tables/table">
						<xsl:if test="@batchInterfaceExport !='false'">
						<SyncSFTPTaskExport outputFileName="{@name}.csv" fieldSeparator=";" filePath="Files/data/FTs/exports/batch" targetDirectory="" header="true">
							<syncSFTPTaskTables>
								<syncSFTPTaskTable history="false" name="{@name}"/>
							</syncSFTPTaskTables>
							<syncSFTPPlan/>
						</SyncSFTPTaskExport>
						</xsl:if>
       ...
       <!-- KONEC podminka, ze aspon jeden export je zaskrtlej -->   
				<xsl:for-each select="syncTasks/syncSFTPTasks/syncSFTPTask">  <!-- put this "syncTasks/syncSFTPTasks/syncSFTPTasksE/syncSFTPTaskE" here 10.2.0 -> 10.x.x -->
			...
			<syncSFTPTasksI>
			<xsl:for-each select="syncTasks/syncSFTPTasksI/syncSFTPTaskI">  <!-- put this "syncTasks/syncSFTPTasks/syncSFTPTasksI/syncSFTPTaskI" here 10.2.0 -> 10.x.x -->
		...
		<syncOnlineTasks>
			<syncOnlineTasksE>		
		<!-- this part delete here 10.2.0 -> 10.x.x and use normal second transformation for Online export -->	
			<!-- podminka, ze aspon jeden online je zaskrtlej -->   
				<xsl:if test="//logicalModel/tables/table[@onlineInterface !='false']">
					<syncOnlineTaskE name="getTable">
						<syncOnlineTaskExports>
							<xsl:for-each select="logicalModel/tables/table">
							<xsl:if test="@onlineInterface !='false'">
							<SyncOnlineTaskExport outputFileName="{@name}">
								<syncOnlineTaskTables>
										<syncOnlineTaskTable history="false" name="{@name}"/>
								</syncOnlineTaskTables>
								<syncOnlineTaskColumns>
									<xsl:for-each select="columns/column">
										<xsl:variable name="domain" select="@domain"/>
										<xsl:choose>
											<xsl:when test="//logicalModel/domains/domain[@name = $domain]/@type = 'LONG'">
												<syncOnlineTaskColumn name="{@name}" type="LONG_INTEGER"/>
											</xsl:when>
											<xsl:when test="//logicalModel/domains/domain[@name = $domain]/@type = 'MNREFERENCES'">
												<syncOnlineTaskColumn name="{@name}" type="STRING"/>
											</xsl:when>
											<xsl:otherwise>
												<syncOnlineTaskColumn name="{@name}" type="{//logicalModel/domains/domain[@name = $domain]/@type}"/>
											</xsl:otherwise>
										</xsl:choose>											
									</xsl:for-each>
								</syncOnlineTaskColumns>
       ...
       <!-- KONEC podminka, ze aspon jeden online je zaskrtlej -->  				
...
<!-- this part mustbe changed here 10.2.0 -> 10.x.x - leave only  auditing element from first part -->	
	<xsl:choose>
		<xsl:when test="auditing">
		...
		<xsl:otherwise>
			<auditing limit="100000" append="true" pattern="rdm-audit%g.log" count="4">
				<auditableActions dataRead="true" dataModification="true" dataExport="true" securityModification="true" workflowAction="true" systemEvent="true"/>
			</auditing>
		</xsl:otherwise>
	</xsl:choose>
</metadata>
