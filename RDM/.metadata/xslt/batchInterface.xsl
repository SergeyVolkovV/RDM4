...
<!-- (parameters) -->
    <!--
		...
		<connection className="com.ataccama.dqc.model.elements.connections.StandardFlowConnection">
			...
			<target endpoint="parameters" step="Rdm Importer"/>
		</connection>
    -->
    ...
    <step id="Rdm Importer" className="com.ataccama.rdm.manager.steps.RdmImportStep" disabled="false" mode="NORMAL">
        <properties mrg:retainNodes="allowedEditStates,importTable,moveToEditAction,incremental" tablename="{@name}" url="{$rdmAppVariablesRef/@connectionName}">
        	<xsl:choose>
            ...
            <columns>
            	<mrg:mergeChildren key="@name">   
		            <xsl:for-each select="columns/column[not(@name=$idGenerators/generator/@name)]">
	            		...
	            		<rdmImportStepColumnMapping name="{@name}" type="{upper-case($tmpType)}" inColumn="{@name}" mrg:retainNodes="inColumn"/>
		            </xsl:for-each>
	            </mrg:mergeChildren>
            </columns>            
        </properties>
    ...
    <step id="Errors" className="com.ataccama.dqc.tasks.io.text.write.TextFileWriter" disabled="false" mode="NORMAL">
        <properties useStringQualifierOnAllColumns="false" stringQualifier="&quot;" lineSeparator="\n" fieldSeparator=";" generateMetadata="true" fileName="errors/{@name}.txt" encoding="UTF-8" writeHeader="true" writeAllColumns="true" stringQualifierEscape="&quot;">
            <columns/>
